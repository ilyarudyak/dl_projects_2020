...
#################### about ####################
# in this file we use custom loss function
# instead of class_weights in model.fit
...
class CheXNetDataGen:
    ...
    def _clean_df(self):
        ...
        for label in self.LABELS:
            self.df[label] = self.df[self.FINDING_LABELS].str.contains(label).astype(np.int)
...
class Trainer:
    ...
    def __init__(self,
        ...
        if not is_fine_tune:
            self.experiment_dir = experiment_dir / 'pneumonia_base_model'
        else:
            self.experiment_dir = experiment_dir / 'pneumonia_base_model_ft'
        ...
        ##### metrics and loss
        self.model.compile(loss=get_weighted_loss(pos_weight=self.params.dict['weight_positive'],
    ...
    def evaluate(self, is_verbose=False):
        ...
        # we use sparce labels here: [0, 1, 0, ... ]
